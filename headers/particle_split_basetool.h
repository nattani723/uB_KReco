...
namespace Kaon_Analyzer
{
  ...
  //void Kaon_Analyzer::CCKaonAnalyzerRebuild::ClearMCTruths(){
  //}
  ...
  */
  ...
  const double growing_fit_initial_length_munu = 10.;
  const double initial_fit_distance_to_line_munu = 1;

  const double growing_fit_initial_length_pip = 10.;
  const double initial_fit_distance_to_line_pip = 1;

  /*
  const double growing_fit_initial_length_munu = 10.;
  const double initial_fit_distance_to_line_munu = 0.5;

  const double growing_fit_initial_length_pip = 5.;
  const double initial_fit_distance_to_line_pip = 0.25;
  */

  //const int min_initial_hits_found = 10;
  //const int max_fitting_hits = 25;
  const int min_initial_hits_found = 7;
  ...
  //const double growing_fit_segment_length = 2.5;
  ...
  */
  ...
  const double distance_to_line_spineall = 0.75;
  const double hit_connection_distance_pip_spineall = 5;

  const double distance_to_line_micro = 0.75;
  ...
  //const double hit_connection_distance = 1.;

  void Kaon_Analyzer::CCKaonAnalyzerRebuild::fillAngularDistributionMapCheat(std::vector<art::Ptr<recob::Hit>>& hits_from_pfparticle,
  ...
  void Kaon_Analyzer::CCKaonAnalyzerRebuild::fillAngularDistributionMapCheat3D(std::vector<art::Ptr<recob::Hit>>& hits_from_pfparticle,
  ...
  void CCKaonAnalyzerRebuild::fillAngularDistributionMap( std::vector<art::Ptr<recob::Hit>>& hits_from_pfparticle, 
						   TVector3 Kend_candidate,
						   art::FindManyP<recob::SpacePoint>& spacepoint_per_hit,
  ...
  void CCKaonAnalyzerRebuild::fillAngularDistributionMap3D( std::vector<art::Ptr<recob::Hit>>& hits_from_pfparticle, 
						     TVector3 Kend_candidate,
						     art::FindManyP<recob::SpacePoint>& spacepoint_per_hit,
  ...
  void CCKaonAnalyzerRebuild::smoothAngularDistributionMapCheat3D(std::map<int, std::map<int, std::map<int, double>>> &angular_distribution_mrgid_map_3D){
  ...
  void CCKaonAnalyzerRebuild::smoothAngularDistributionMap3D(std::map<int, std::map<int, double>> &angular_distribution_map_3D){
  ...
  void CCKaonAnalyzerRebuild::smoothAngularDistributionMap(std::map<int, double> &angular_distribution_map){
  ...
  void CCKaonAnalyzerRebuild::obtainPeakVectorCheat3D(std::map<int, std::map<int, std::map<int, double>>> &angular_distribution_mrgid_map_3D,
					       std::map<int,int>& mrgidpdg,
  ...
  void CCKaonAnalyzerRebuild::accumulateAngularDistributionMap3D(std::map<int, std::map<int, double>> &angular_distribution_map_3D_1,
  ...
  void CCKaonAnalyzerRebuild::obtainPeakVector3D(std::map<int, std::map<int, double>> &angular_distribution_map_3D,
  ...
  void CCKaonAnalyzerRebuild::findBestAngularPeak3D(std::map<int, std::map<int, double>> &angular_distribution_map_3D,
					     std::map<double, TVector2, std::greater<>>& view_peak_map,
	...
	//if(open_angle > TMath::Pi()*2/3) best_peak_bins.push_back(theta_phi_bin);
  ...
  */
  ...
  void CCKaonAnalyzerRebuild::findBestAngularPeakCheat3D( std::map<int, std::map<double, TVector2, std::greater<>>>& view_peak_map_cheat,
    ...
    if(view_peak_map_cheat.size()==0) return;
  ...
  void CCKaonAnalyzerRebuild::findShowerSpine3D(std::vector<art::Ptr<recob::Hit>>& hits_from_pfparticle,
					 art::FindManyP<recob::SpacePoint>& spacepoint_per_hit,
    ...
    for(size_t i_h=0; i_h<hits_from_pfparticle.size(); i_h++){
      ...
      //shower_spine_hit_list.push_back(hits_from_pfparticle[i_h]); //add all inputs as shower spine hits
	...
	if(!hits_collected){
	cout << "!hits_collected, fit with all spine hit list" << endl;
	...
	hit_connection_distance = hit_connection_distance_pip_spineall;
	  ...
	  if(count == 1)
	    ...
	    cout << "extrapolated_direction of all spine: " << extrapolated_direction.X() << " " << extrapolated_direction.Y() << " " << extrapolated_direction.Z() <<endl;
	...
	const lar_content::ThreeDSlidingFitResult micro_extrapolated_fit(&pandora_running_fit_position_micro_vec, 5., sliding_fit_pitch);
	    ...
	    cout << "extrapolated_direction of micro fit: " << extrapolated_direction.X() << " " << extrapolated_direction.Y() << " " << extrapolated_direction.Z() <<endl;
  ...
  void CCKaonAnalyzerRebuild::findShowerSpine3D(std::vector<art::Ptr<recob::Hit>>& hits_from_pfparticle,
    ...
    for(size_t i_h=0; i_h<hits_from_pfparticle.size(); i_h++){
      ...
      cout << "l: " << l << ", t: " << t << ", XYZ " << hit_position.X() << " " << hit_position.Y() << " " << hit_position.Z() << endl;
    

      //if( (best_peak_bins.size()==1 &&  (l<growing_fit_initial_length_munu) && (l>0.) && (t<l*0.20)) ||
      //  (best_peak_bins.size()>1 &&  (l<growing_fit_initial_length_pip) && (l>0.) && (t<l*0.20)) ){

      if( (best_peak_bins.size()==1 &&  (l<growing_fit_initial_length_munu) && (l>0.) && (t<initial_fit_distance_to_line_munu)) ||
	  (best_peak_bins.size()>1 &&  (l<growing_fit_initial_length_pip) && (l>0.) && (t<initial_fit_distance_to_line_pip)) ){

	cout << "------------------------------ this hit has passed iniital cut -------------------------" << endl;
    ...
    if(best_peak_bins.size()==1){
      distance_to_line = distance_to_line_munu;
      hit_connection_distance = hit_connection_distance_munu;
      max_fitting_hits = max_fitting_hits_munu;
    }
    if(best_peak_bins.size()>=1){
      distance_to_line = distance_to_line_pip;
      hit_connection_distance = hit_connection_distance_pip;
      max_fitting_hits = max_fitting_hits_pip;
    }

    cout << "the number of hits in initial shower spine is " << shower_spine_hit_list.size() << endl;  
    cout << "running_fit_position_vec.size(): " << running_fit_position_vec.size() << endl;
    cout << "pandora_running_fit_position_vec.size(): " << pandora_running_fit_position_vec.size() << endl;
    ...
    if( (Kend_candidate - closest_hit).Mag()>10 ){
      shower_spine_hit_list.clear();
      shower_spine_hit_list_vector.push_back(shower_spine_hit_list);
      continue;
    }
	...
	if(excess_hits_in_fit>0){
	  ...
	  /*
	  if(count!=1){
	    std::reverse(pandora_running_fit_position_vec.begin(), pandora_running_fit_position_vec.end());
	    for(auto vec: pandora_running_fit_position_vec){
	  ...
	  */
	    ...
	    for (int i = 0; i < excess_hits_in_fit; ++i){
	      //running_fit_position_vec.erase(std::prev(running_fit_position_vec.end()));
	      if(count==1) pandora_running_fit_position_vec.erase(std::prev(pandora_running_fit_position_vec.end()));
	      else{
		//cout << "removing from run fit vector begin " << pandora_running_fit_position_vec[0].GetX() << " " << pandora_running_fit_position_vec[0].GetY()  << " " << pandora_running_fit_position_vec[0].GetZ() << endl;
		//pandora_running_fit_position_vec.erase(std::next(pandora_running_fit_position_vec.begin()));
		pandora_running_fit_position_vec.erase(std::prev(pandora_running_fit_position_vec.end()));
	      }
	    }
	  ...
	  if(count!=1){
	    ...
	    */
	    ...
	    for(auto pandora_running_fit_position : pandora_running_fit_position_vec){
	      hit_position_tmp.SetXYZ(pandora_running_fit_position.GetX(), pandora_running_fit_position.GetY(), pandora_running_fit_position.GetZ());
	      running_fit_position_vec.push_back(hit_position_tmp);
	      cout << "pandora_running_fit_position: " << pandora_running_fit_position.GetX() << " " << pandora_running_fit_position.GetY() << " " << pandora_running_fit_position.GetZ() << endl;
	    }
	...
	//if(count>1) cout << "this track has more than 1 count" << endl;
	//if(count==1) cout << "this track has 1 count" << endl;
	...
	if(count == 1){
	  ...
	  if(is_end_downstream){
	    ...
	    //cout << "extrapolated_fit.GetGlobalMinLayerPosition().GetX(): " << extrapolated_fit.GetGlobalMinLayerPosition().GetX() << endl;
	    extrapolated_start_position.SetXYZ(extrapolated_fit.GetGlobalMinLayerPosition().GetX(), extrapolated_fit.GetGlobalMinLayerPosition().GetY(), extrapolated_fit.GetGlobalMinLayerPosition().GetZ());
	  }
	}
	...
	if(is_end_downstream){
	  ...
	  cout << "is_end_downstream: " << is_end_downstream << " " << extrapolated_fit.GetGlobalMaxLayerDirection().GetX() << " " << extrapolated_fit.GetGlobalMaxLayerDirection().GetY() << " " << extrapolated_fit.GetGlobalMaxLayerDirection().GetZ() << " open angle " << initial_direction.Angle(extrapolated_direction) << endl;
	  ...
	  cout << "is_end_downstream: " << is_end_downstream << " " << extrapolated_direction.X()  << " " << extrapolated_direction.Y() << " " << extrapolated_direction.Z()  << " open angle " << initial_direction.Angle(extrapolated_direction) << endl;
	  ...
	  //cout << "is_end_downstream: " << is_end_downstream << " " << extrapolated_fit.GetGlobalMinLayerDirection().GetX() << " " << extrapolated_fit.GetGlobalMinLayerDirection().GetY() << " " << extrapolated_fit.GetGlobalMinLayerDirection().GetZ() << endl;

	}

	extrapolated_end_position = extrapolated_start_position + (extrapolated_direction * growing_fit_segment_length);
	cout << "extrapolated_direction: " << extrapolated_direction.X() << " " << extrapolated_direction.Y() << " " << extrapolated_direction.Z() <<endl;
	...
	if(!hits_collected){
	cout << "flg_spineall" << flg_spineall << endl;
	//if(!hits_collected && count!=1){
	  //if(!hits_collected && count!=1 && flg_spineall==true){
	cout << "!hits_collected, fit with all spine hit list" << endl;
	//if(count == 1) return;
	//if(flg_spineall == false) return;
	flg_spineall = false;
	...
	hit_connection_distance = hit_connection_distance_pip_spineall;
	  ...
	  //std::reverse(pandora_running_fit_position_spineall_vec.begin(), pandora_running_fit_position_spineall_vec.end());
	  ...
	  if(count == 1)
	    ...
	    if(is_end_downstream){
	      ...
	      cout << "is_end_downstream: " << is_end_downstream << " " << spineall_extrapolated_fit.GetGlobalMaxLayerDirection().GetX() << " " << spineall_extrapolated_fit.GetGlobalMaxLayerDirection().GetY() << " " << spineall_extrapolated_fit.GetGlobalMaxLayerDirection().GetZ() << " open angle " << initial_direction.Angle(extrapolated_direction) << endl;
	  ...
	  //cout << "is_end_downstream: " << is_end_downstream << " " << spineall_extrapolated_fit.GetGlobalMaxLayerDirection().GetX() << " " << spineall_extrapolated_fit.GetGlobalMaxLayerDirection().GetY() << " " << spineall_extrapolated_fit.GetGlobalMaxLayerDirection().GetZ() << endl;
	  ...
	  cout << "is_end_downstream: " << is_end_downstream << " " << spineall_extrapolated_fit.GetGlobalMinLayerDirection().GetX() << " " << spineall_extrapolated_fit.GetGlobalMinLayerDirection().GetY() << " " << spineall_extrapolated_fit.GetGlobalMinLayerDirection().GetZ() << endl;

	    }

	    extrapolated_end_position = extrapolated_start_position + (extrapolated_direction * growing_fit_segment_length);
	    //extrapolated_end_position = extrapolated_start_position + (extrapolated_direction * 7.5);
	    cout << "extrapolated_direction: " << extrapolated_direction.X() << " " << extrapolated_direction.Y() << " " << extrapolated_direction.Z() <<endl;
	...
	if (!hits_collected)
	  {
	cout << "!hits_collected, reduce the sliding window" << endl;
	...
	cout << "is_end_downstream: " << is_end_downstream << endl;
	    ...
	    if(is_end_downstream){
	      ...
	      cout << "is_end_downstream: " << is_end_downstream << " " << micro_extrapolated_fit.GetGlobalMaxLayerDirection().GetX() << " " << micro_extrapolated_fit.GetGlobalMaxLayerDirection().GetY() << " " << micro_extrapolated_fit.GetGlobalMaxLayerDirection().GetZ() << " open angle " << initial_direction.Angle(extrapolated_direction) << endl;
	  ...
	  //cout << "is_end_downstream: " << is_end_downstream << " " << micro_extrapolated_fit.GetGlobalMaxLayerDirection().GetX() << " " << micro_extrapolated_fit.GetGlobalMaxLayerDirection().GetY() << " " << micro_extrapolated_fit.GetGlobalMaxLayerDirection().GetZ() << endl;
		...
		is_end_downstream = true;
	      ...
	      cout << "is_end_downstream: " << is_end_downstream << " " << micro_extrapolated_fit.GetGlobalMinLayerDirection().GetX() << " " << micro_extrapolated_fit.GetGlobalMinLayerDirection().GetY() << " " << micro_extrapolated_fit.GetGlobalMinLayerDirection().GetZ() << endl;
    ...
    cout << "final shower_spine_hit_list" << endl; 
    ...
    for(size_t i_h=0; i_h<shower_spine_hit_list.size(); i_h++){
      ...
      cout << "hit_position: " << hit2_position.X() << " " << hit2_position.Y() << " " <<  hit2_position.Z() << " " <<  endl;
    }
  ...
  void CCKaonAnalyzerRebuild::findShowerSpine3D(std::vector<art::Ptr<recob::Hit>>& hits_from_pfparticle,
    ...
    // Require significant number of initial hits
    ini_hits = running_fit_position_vec.size();
    if (running_fit_position_vec.size() < min_initial_hits_found){
      cout << "Requiring significant number of initial hits" << endl;
      shower_spine_hit_list.clear();
      shower_spine_hit_list_vector.push_back(shower_spine_hit_list); 
    ...
    closest_distance = (Kend_candidate - closest_hit).Mag();
    if( (Kend_candidate - closest_hit).Mag()>10 ){
      shower_spine_hit_list.clear();
      shower_spine_hit_list_vector.push_back(shower_spine_hit_list);
      continue;
    ...
    cout << "the number of hits in shower spine is " << shower_spine_hit_list.size() << endl;
    num_hits = shower_spine_hit_list.size();
    if(shower_spine_hit_list.size()<30){
      cout << "Requiring significant number of final hits" << endl;
      shower_spine_hit_list.clear();
      shower_spine_hit_list_vector.push_back(shower_spine_hit_list);
  ...
  void CCKaonAnalyzerRebuild::findShowerSpine3D(std::vector<art::Ptr<recob::Hit>>& hits_from_pfparticle,
	...
	cout << "before collectSubsectionHits" << endl;
	hits_collected = this->collectSubsectionHits(extrapolated_fit, extrapolated_start_position, extrapolated_end_position, extrapolated_direction, is_end_downstream, hits_from_pfparticle, spacepoint_per_hit, running_fit_position_vec, pandora_running_fit_position_vec, unavailable_hit_list, shower_spine_hit_list, Kend_candidate, true_length);
	cout << "after collectSubsectionHits" << endl;
  ...
  */
  ...
  bool CCKaonAnalyzerRebuild::collectSubsectionHits(const lar_content::ThreeDSlidingFitResult &extrapolated_fit,
					     ...
					     std::vector<art::Ptr<recob::Hit>>& shower_spine_hit_list)
  {
    ...
    lar_content::LArConnectionPathwayHelper::SortByDistanceToPoint smpl(pandora_extrapolated_start_position);//replace with pandora_running_fit_position_vec[0]?
    ...
    cout << "extrapolated_start_l: " << extrapolated_start_l << ", extrapolated_end_l: " << extrapolated_end_l << endl;
    for(size_t i_h=0; i_h<hits_from_pfparticle_sort.size(); i_h++){
      ...
      //7.67785 -63.8507 321.416
      //if(hit_position.X() <= 11) cout << "hit_position.X() " << hit_position.X() <<  ", hit_l " << hit_l << ", hit_t " << hit_t << ", hit_t2 " << hit_t2 << endl;
    ...
    //vector<TVector3> running_fit_position_vec_tmp;
    //pandora::CartesianPointVector pandora_running_fit_position_vec_tmp;
    std::reverse(running_fit_position_vec.begin(), running_fit_position_vec.end());
    std::reverse(pandora_running_fit_position_vec.begin(), pandora_running_fit_position_vec.end());
  ...
  void CCKaonAnalyzerRebuild::collectConnectedHits(std::vector<art::Ptr<recob::Hit>>& collected_hits,
					    art::FindManyP<recob::SpacePoint>& spacepoint_per_hit,
					    const TVector3 &extrapolated_start_position,
					    const TVector3 &extrapolated_direction,
					    vector<TVector3> &running_fit_position_vec,
					    ...
					    std::vector<art::Ptr<recob::Hit>>& shower_spine_hit_list)
  {
	...
	for(size_t i_h=0; i_h<collected_hits.size(); i_h++){
	  ...
	  //cout << "connect running_fit_position_vec" << endl;
	  //this->getClosestDistance(hit_position, running_fit_position_vec);
	  //cout << "connect shower_spine_hit_list" << endl;
	  //this->getClosestDistance(hit_position, shower_spine_hit_list_position);
	  //getClosestDistance(collected_hits[i_h], shower_spine_hit_list, spacepoint_per_hit);
	  

	  //cout << "connect running_fit_position_vec is " << this->getClosestDistance(hit_position, running_fit_position_vec) << " and hit_connection_distance is " << hit_connection_distance << ", so true/false filter is " << (this->getClosestDistance(hit_position, running_fit_position_vec) > hit_connection_distance) << endl;
	  if(this->getClosestDistance(hit_position, running_fit_position_vec) > hit_connection_distance){
	    ...
	    spacepoint_vec_1 = spacepoint_per_hit.at(collected_hits.key());
	    const TVector3 hit1_position = spacepoint_vec_1[0]->XYZ();
	    */
	    /*
	    separation = this->getClosestDistance(hit1_position, shower_spine_hit_list_position);
	    if(separation < closest_distance) closest_distance = separation;
	    */
	    //cout << "connect shower_spine_position_vec is " << this->getClosestDistance(hit_position, shower_spine_hit_list_position) << " and hit_connection_distance is " << hit_connection_distance << ", so true/false filter is " << (this->getClosestDistance(hit_position, shower_spine_hit_list_position) > hit_connection_distance) << endl;
	  ...
	  found = true;
	  //cout << "IN collectConnectedHits" << endl;
	  //cout << "running_fit_position_vec.size(): " << running_fit_position_vec.size() << endl;
	  running_fit_position_vec.push_back(hit_position);
	  pandora_running_fit_position_vec.push_back(pandora_hit_position);
	  //cout << "running_fit_position_vec.size(): " << running_fit_position_vec.size() << endl;
  ...
  TVector3 CCKaonAnalyzerRebuild::getClosestPointToLine3D(const TVector3 &extrapolated_start_position,
						   const TVector3 &extrapolated_direction,
  ...
  bool CCKaonAnalyzerRebuild::isCloseToLine(const TVector3 &hit_position,
  ...
  double CCKaonAnalyzerRebuild::getClosestDistance(art::Ptr<recob::Hit>& collected_hit,
					    ...
					    art::FindManyP<recob::SpacePoint>& spacepoint_per_hit)
  {
    ...
    //cout << "mod_delta_z: " << mod_delta_z << endl;
  ...
  double CCKaonAnalyzerRebuild::getClosestDistance(const TVector3 &position,
					    vector<TVector3> &test_positions)
  {
    ...
    cout << "closest_distance: " << std::sqrt(closest_distance_sqaured) << endl;
  ...
  void CCKaonAnalyzerRebuild::obtainLength(std::vector<art::Ptr<recob::Hit>>& shower_spine_hit_list,
				    art::FindManyP<recob::SpacePoint>& spacepoint_per_hit,
				    TVector3 Kend_candidate)
  {
  ...
  void CCKaonAnalyzerRebuild::obtainLongitudinalDecomposition(std::vector<art::Ptr<recob::Hit>>& shower_spine_hit_list,
						       art::FindManyP<recob::SpacePoint>& spacepoint_per_hit)
  {
  ...
  int CCKaonAnalyzerRebuild::fillHistAngularDistributionMapCheat( std::map<int, std::map<int, double>> &angular_distribution_mrgid_map,
							   std::map<int,int> &mrgidpdg,
  ...
  int CCKaonAnalyzerRebuild::fillHistAngularDistributionMapCheat3D( std::map<int, std::map<int, std::map<int, double>>> &angular_distribution_mrgid_map_3D,
							     std::map<int,int> &mrgidpdg,
  ...
  int CCKaonAnalyzerRebuild::fillHistAngularDistributionMap( std::map<int, double> &angular_distribution_map,
  ...
  int CCKaonAnalyzerRebuild::fillHistAngularDistributionMap3D( std::map<int, std::map<int, double>> &angular_distribution_map_3D,
  ...
  int CCKaonAnalyzerRebuild::fillHistAngularDistributionMapSurface( std::map<int, std::map<int, double>> &angular_distribution_map_3D,
  ...
  int CCKaonAnalyzerRebuild::fillHistAngularDistributionMapSphere( std::map<int, std::map<int, double>> &angular_distribution_map_3D,
  ...
  int CCKaonAnalyzerRebuild::drawHistAngularDistributionMapCheat( std::vector<TH1D*> &h_angular_distribution_pfparticle_cheat,
  ...
  int CCKaonAnalyzerRebuild::drawHistAngularDistributionMapCheat3D( std::vector<TH2D*> &h_angular_distribution_pfparticle_cheat_3D,
							     std::vector<int> &v_pdg,
							     TString outfile_name,
							     TCanvas* &c){
  ...
  int CCKaonAnalyzerRebuild::drawHistAngularDistributionMap( std::vector<TH1D*> &h_angular_distribution_pfparticle,
  ...
  int CCKaonAnalyzerRebuild::drawHistAngularDistributionMap3D( std::vector<TH2D*> &h_angular_distribution_pfparticle_3D,
							std::vector<bool> &v_trk_flg,
							TString outfile_name,
							TCanvas* &c){
  ...
  int CCKaonAnalyzerRebuild::drawHistAngularDistributionMapSurface( std::vector<TH2D*> &h_angular_distribution_pfparticle_surface,
							     std::vector<bool> &v_trk_flg,
							     TString outfile_name,
							     TCanvas* &c){
  ...
  int CCKaonAnalyzerRebuild::drawHistAngularDistributionMapSphere( std::vector<TH3D*> &h_angular_distribution_pfparticle_sphere,
							    std::vector<bool> &v_trk_flg,
							    TString outfile_name,
							    TCanvas* &c){
    TFile outfile(outfile_name, "update");
