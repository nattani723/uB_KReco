...
{
  void Kaon_Analyzer::CCKaonAnalyzerRebuild::ClearSlices(){
        m_reco_slice_num = 0;
    ...
    void CCKaonAnalyzerRebuild::ResizeSlices(size_t size){
    ...
    */
    ...
    void CCKaonAnalyzerRebuild::CreateSliceBranches(){
	...
	vertex_tree->Branch("PFP_have_nuslice",& PFP_have_nuslice);
    ...
    /*
       void CCKaonAnalyzerRebuild::CreateMatchedSliceBranches(){
    ...
    void CCKaonAnalyzerRebuild::AnalyzeSlices(std::map<art::Ptr<recob::PFParticle>, std::vector<art::Ptr<larpandoraobj::PFParticleMetadata>> > & pfParticleToMetadataMap,
            ...
            if (!metadatalist.empty()){
                ...
                for(art::Ptr<larpandoraobj::PFParticleMetadata> data:metadatalist){
                    ...
                    if(temp_score != -1.0){
                        ...
                        if(m_is_verbose)std::cout<<"CCKaonAnalyzerRebuild::AnalyzeSlice()\t||\t found primary PFP at index "<<pfp->Self()<<std::endl;
        ...
        */
        ...
        std::cout<<"CCKaonAnalyzerRebuild::AnalyzeSlice()\t||\t the number of clear cosmic PFP's in the event is "<<clearCosmicPFP.size()<<std::endl;
        std::cout<<"CCKaonAnalyzerRebuild::AnalyzeSlice()\t||\t the number of items in the primary to slice vector is "<<primaryPFPSliceIdVec.size()<<" and in the slice to score map is "<<sliceIdToNuScoreMap.size()<<std::endl;
        std::cout<<"CCKaonAnalyzerRebuild::AnalyzeSlice()\t||\t the number of PFP's matched to a slice is "<< allPFPSliceIdVec.size()<<"/"<< pfParticleMap.size()<<std::endl;
    ...
    //  void  CCKaonAnalyzerRebuild::GetPFPsPerSlice( std::map<art::Ptr<recob::PFParticle>, int>& PFPToSliceIdMap, std::vector<int> &sliceIdToNumPFPsvec){
    std::vector<int>  CCKaonAnalyzerRebuild::GetPFPsPerSlice( std::map<art::Ptr<recob::PFParticle>, int>& PFPToSliceIdMap){
    ...
    std::vector<int> CCKaonAnalyzerRebuild::GetNumShowersPerSlice(std::map< art::Ptr<recob::Shower>,art::Ptr<recob::PFParticle>>& showerToPFParticleMap, 
            std::map<art::Ptr<recob::PFParticle>, int>& PFPToSliceIdMap){
    ...
    std::vector<int> CCKaonAnalyzerRebuild::GetNumTracksPerSlice(std::map< art::Ptr<recob::Track>,art::Ptr<recob::PFParticle>>& trackToPFParticleMap, 
            std::map<art::Ptr<recob::PFParticle>, int>& PFPToSliceIdMap){
    ...
    int CCKaonAnalyzerRebuild::GetShowerSlice(art::Ptr<recob::Shower>& this_shower, std::map< art::Ptr<recob::Shower> , art::Ptr<recob::PFParticle>>& showerToPFParticleMap, std::vector<std::pair<art::Ptr<recob::PFParticle>,int>> & allPFPSliceIdVec){
    ...
    int CCKaonAnalyzerRebuild::GetTrackSlice(art::Ptr<recob::Track>& this_track, std::map< art::Ptr<recob::Track> , art::Ptr<recob::PFParticle>>& trackToPFParticleMap, std::vector<std::pair<art::Ptr<recob::PFParticle>,int>> & allPFPSliceIdVec){
    ...
    void CCKaonAnalyzerRebuild::AnalyzeRecoMCSlices(std::string signal_def, std::map<int, art::Ptr<simb::MCParticle>> & MCParticleToTrackIDMap,
	...
	/*
        ...
        if(signal_def == "cckaon"){
            //commenting out this requirement since it's different in v13
            if(m_mctruth_is_delta_radiative || !m_mctruth_is_delta_radiative){
                std::cout<<"CCKaonAnalyzerRebuild::AnalyzeSlice()\t||\t looking for signal def "<<signal_def<<", m_mctruth_is_delta_radiative = "<<m_mctruth_is_delta_radiative<<std::endl; 
	...
	*/
        ...
        std::cout<<"CCKaonAnalyzerRebuild::AnalyzeSlice()\t||\t AnalyzeRecoMCSlices is done "<<std::endl;
    ...
    //loops over association between reco tracks/showers to get associated slice(s)
    //can also look at things like shower energy, conversion length, etc.
    void CCKaonAnalyzerRebuild::FindSignalSlice(std::string signal_def, std::map<int, art::Ptr<simb::MCParticle>> & MCParticleToTrackIDMap,
            std::map<art::Ptr<recob::Shower>,art::Ptr<recob::PFParticle> > & showerToPFParticleMap, 
            std::vector<std::pair<art::Ptr<recob::PFParticle>,int>> & allPFPSliceIdVec, 
            std::map<art::Ptr<recob::Shower>, art::Ptr<simb::MCParticle> > & showerToMCParticleMap,
            std::map<art::Ptr<recob::Track>,art::Ptr<recob::PFParticle> > & trackToNuPFParticleMap,
        ...
        if(signal_def == "ncdelta"){
            ...
            if(m_mctruth_is_delta_radiative== true){
                ...
                if(m_is_verbose) std::cout<<"CCKaonAnalyzerRebuild::AnalyzeSlice()\t||\t the number of sim showers-MCP matches associated to the true ncdelta is "<<matched_reco_slice_shower_MCP.size()<<std::endl;
                ...
                std::cout<<"CCKaonAnalyzerRebuild::AnalyzeSlice()\t||\t the number of sim tracks-MCP matches associated to the true ncdelta is "<<matched_reco_slice_track_MCP.size()<<std::endl;
                //check if either 1g1p or 1g0p topology
                std::cout<<"Status - ";
