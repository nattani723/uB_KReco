...
{
  void Kaon_Analyzer::CCKaonAnalyzerRebuild::ClearFlashes(){
    ...
    void CCKaonAnalyzerRebuild::ResizeFlashes(size_t size){
    ...
    void CCKaonAnalyzerRebuild::CreateFlashBranches(){
    ...
    void CCKaonAnalyzerRebuild::AnalyzeFlashes(const std::vector<art::Ptr<recob::OpFlash>>& flashes, art::Handle<std::vector<crt::CRTHit>> crthit_h, double evt_timeGPS_nsec){

        //  void CCKaonAnalyzerRebuild::AnalyzeFlashes(const std::vector<art::Ptr<recob::OpFlash>>& flashes, art::Handle<std::vector<crt::CRTHit>> crthit_h){

        size_t flash_size = flashes.size();
        m_reco_num_flashes = flash_size;

        if(m_is_verbose) std::cout<<"CCKaonAnalyzerRebuild::AnalyzeFlashes()\t||\t Beginning analysis of recob::OpFlash\n";
        ...
        if(m_is_verbose) std::cout<<"CCKaonAnalyzerRebuild::AnalyzeFlashes()\t||\t Finished. There was "<<flash_size<<" flashes with: "<<m_reco_num_flashes_in_beamgate<<" in the beamgate defined by: "<<m_beamgate_flash_start<<" <-> "<<m_beamgate_flash_end<<std::endl;
