...
namespace Kaon_Analyzer
{
  ...
  void Kaon_Analyzer::CCKaonAnalyzer::ClearSecondShowers(){
    ...
    void CCKaonAnalyzer::ResizeSecondShowers(size_t size){

    }


    void CCKaonAnalyzer::CreateSecondShowerBranches(){
    ...
    void CCKaonAnalyzer::SecondShowerSearch(
        ...
        for(size_t t =0; t< tracks.size(); t++){
            ...
            std::cout<<"CCKaonAnalyzer::SSS\t||\ttrack "<<t<<" is in slice "<<sliceid<<" which has "<<slicehits.size()<<" hits. This track has  "<<trackhits.size()<<" of them. "<<std::endl;
        ...
        for(size_t s =0; s< showers.size(); s++){
            ...
            std::cout<<"CCKaonAnalyzer::SSS\t||\tshower "<<s<<" is in slice "<<sliceid<<" which has "<<slicehits.size()<<" hits. This shower has  "<<showerhits.size()<<" of them. "<<std::endl;
        ...
        std::cout<<"CCKaonAnalyzer::SSS\t||\tSo in total we have "<<total_shower_hits<<" shower hits and "<<total_track_hits<<" track hits"<<" assocaedHits total: "<<associated_hits.size()<<std::endl;
        m_sss_num_associated_hits = total_shower_hits+total_track_hits;

        if(nu_slice_id>=0){
            std::cout<<"CCKaonAnalyzer::SSS\t||\t So that leaves "<<sliceIDToHitsMap.at(nu_slice_id).size()-total_shower_hits-total_track_hits<<" hits not included in tracks and showers"<<std::endl;
            ...
            std::cout<<"CCKaonAnalyzer::SSS\t||\tassociated_hits.size() "<<associated_hits.size()<<" unassociated_hits.size() "<<unassociated_hits.size()<<" p0: "<<unassociated_hits_plane0.size()<<" p1:  "<<unassociated_hits_plane1.size()<<" p2: "<<unassociated_hits_plane2.size()<<std::endl;
            ...
            std::cout<<"CCKaonAnalyzer::SSS\t||\tTick Min: "<<tick_min<<" Max: "<<tick_max<<std::endl;
            auto const TPC = (*geom).begin_TPC();
            auto ID = TPC.ID();
            int fCryostat = ID.Cryostat;
            int fTPC = ID.TPC;
            std::cout<<"CCKaonAnalyzer::SSS\t||\t" << TPC.ID() << "= the beginning TPC ID" <<std::endl;
            std::cout<<"CCKaonAnalyzer::SSS\t||\tthe cryostat id = " << fCryostat << std::endl;  
            std::cout<<"CCKaonAnalyzer::SSS\t||\tthe tpc id = " << fTPC << std::endl;  
            ...
            for(int i=0; i<3; i++){
                ...
                std::cout<<"CCKaonAnalyzer::SSS\t||\tStarting to run DBSCAN for plane: "<<i<<" has "<<pts_to_recluster[i].size()<<" pts to do using eps: "<<eps<<" and min_pts: "<<min_pts<<std::endl; 
                ...
                std::cout << "CCKaonAnalyzer::SSS\t||\tOn this plane, DBSCAN found: "<<num_clusters[i]<<" clusters"<<std::endl;
            }
            ...
            for(int i=0; i<3; i++){
                ...
                if(cluster_labels[i].size() != pts_to_recluster[i].size()){
                    std::cout<<"CCKaonAnalyzer::SSS\t||\tERROR!! someting amiss cluster labels of size "<<cluster_labels[i].size()<<" and pts  in this plane "<<pts_to_recluster[i].size()<<std::endl;
                }  
            ...
            */
            ...
            if(false){
                std::cout<<"CCKaonAnalyzer::SSS\t||\tDone Plotting clusters"<<std::endl;
                can->Update();
                //can->Write();
                can->SaveAs((print_name+".pdf").c_str(),"pdf");
                //f->Close();
                std::cout<<"CCKaonAnalyzer::SSS\t||\tPRINTING"<<std::endl;
            }
            ...
            std::cout<<"CCKaonAnalyzer::SSS\t||\tNo Neutrino Slice Found/No Showers & tracks"<<std::endl;
    ...
    TGraph* CCKaonAnalyzer::GetNearestNpts(int p, int cl, std::vector<art::Ptr<recob::Hit>> &hitz, double vertex_wire, double vertex_tick, int Npts){
    ...
    sss_score CCKaonAnalyzer::ScoreCluster(int p, int cl, std::vector<art::Ptr<recob::Hit>> &hits, double vertex_wire, double vertex_tick, const art::Ptr<recob::Shower> &shower){
        ...
        //(*eigenvec).Print();
        //(*covar).Print();
        //std::cout<<"CCKaonAnalyzer::SSS\t||\tEigen: "<<score.pca_0<<" "<<score.pca_1<<std::endl;
    ...
    int CCKaonAnalyzer::CompareToShowers(int p ,int cl, std::vector<art::Ptr<recob::Hit>>& hitz,double vertex_wire,double vertex_tick,
    ...
    std::vector<double>CCKaonAnalyzer::SecondShowerMatching(std::vector<art::Ptr<recob::Hit>>& hitz,
            art::FindManyP<simb::MCParticle,anab::BackTrackerHitMatchingData>& mcparticles_per_hit,
            std::vector<art::Ptr<simb::MCParticle>>& mcParticleVector,
            std::map< size_t, art::Ptr<recob::PFParticle>> & pfParticleIdMap,
            std::map< int ,art::Ptr<simb::MCParticle> >  &  MCParticleToTrackIdMap){
        ...
        //            if(reco_verbose)std::cout << "CCKaonAnalyzer::recoMC()\t||\t On Object "<<i<<". The number of MCParticles associated with this PFP is "<<objide.size()<<std::endl;       
        //          if(reco_verbose) std::cout<<"CCKaonAnalyzer::recoMC()\t||\t the fraction of hits from overlay is is "<<fraction_num_hits_overlay<<" ("<<n_not_associated_hits<<"/"<<obj_hits_ptrs.size()<<")"<<std::endl;
        ...
        if(reco_verbose)           std::cout<<"CCKaonAnalyzer::recoMC()\t||\t the number of source mother particles is "<<mother_MCP_map.size()<<" of which : "<<marks_mother_vector.size()<<" are unique!"<<std::endl;
        ...
        if(marks_mother_vector.size()!=0){
            //if(reco_verbose)  std::cout<<"CCKaonAnalyzer::recoMC()\t||\t The `BEST` mother is a "<<marks_mother_vector[best_mother_index]->PdgCode()<<" at "<<best_mother_index<<" on plane: "<<best_mother_plane<<std::endl;
            std::cout<<"CCKaonAnalyzer::recoMC()\t||\t The `BEST` mother is a "<<marks_mother_vector[best_mother_index]->PdgCode()<<" at "<<best_mother_index<<" on plane: "<<best_mother_plane<<std::endl;
            for(int l=0; l<3; l++){
                std::cout<<"CCKaonAnalyzer::recoMC()\t||\t It represents "<<marks_mother_energy_fraction_map[marks_mother_vector[best_mother_index]][l]/total_energy_on_plane[l]*100.0<<"% of the energy on plane: "<<l<<" which is "<<total_energy_on_plane[l] <<std::endl;
            }
        }
