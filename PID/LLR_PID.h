...
bool isHitBtMonteCarlo(const size_t hit_index, const std::unique_ptr<art::FindManyP<simb::MCParticle, anab::BackTrackerHitMatchingData>> &assocMCPart, float en_threshold)
{
   ...
   return found_mc_hit;
	...
	if (!fRecalibrateHits)
	...
	else
	  {
		...
		is_hit_montecarlo.push_back(isHitBtMonteCarlo(tp_index, assocMCPart, fEnergyThresholdForMCHits));
	      }
	    // correct hits
